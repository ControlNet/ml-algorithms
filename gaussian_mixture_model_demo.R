...
source("gaussian_mixture_model.R")
...
group <- GaussianMixtureModel(3)$fit(X, 1100, "hard")

ggplot(X %>% as.data.frame) + geom_point(aes(x=X1, y=X2), color=rgb(group), alpha = 0.75)

ggplot(X %>% as.data.frame) + geom_point(aes(x=X1, y=X2, color=group %>% apply(1, which.max) %>% as.factor))